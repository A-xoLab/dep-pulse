...
describe('CacheManager - Integration Tests', () => {
  ...
  describe('GitHub cache behavior', () => {
    const createGitHubVuln = (id: string): Vulnerability => ({
      id,
      title: 'Test GitHub Vulnerability',
      severity: 'low',
      affectedVersions: '< 2.0.0',
      description: 'Test description',
      references: [],
    ...
    it('should respect TTL for GitHub cache (60 minutes)', async () => {
      const packageName = 'gh-package';
      const version = '1.0.0';
      const vulns = [createGitHubVuln('GHSA-TEST-1234')];
      ...
      try {
        nowSpy.mockReturnValue(baseTime);
        await cacheManager.cacheGitHubVulnerabilities(packageName, version, vulns);

        // Within TTL (just under 60 minutes) should still be cached
        nowSpy.mockReturnValue(baseTime + 59 * 60 * 1000);
        let cached = await cacheManager.getCachedGitHubVulnerabilities(packageName, version);
        expect(cached).not.toBeNull();
        expect(cached?.[0]?.id).toBe('GHSA-TEST-1234');

        // After TTL (beyond 60 minutes) should expire
        nowSpy.mockReturnValue(baseTime + 61 * 60 * 1000);
        cached = await cacheManager.getCachedGitHubVulnerabilities(packageName, version);
        expect(cached).toBeNull();
      } finally {
        nowSpy.mockRestore();
      }
  ...
  describe('Critical vulnerability cache bypass', () => {
    const createTestVuln = (
      id: string,
      severity: 'critical' | 'high' | 'medium' | 'low'
    ): Vulnerability => ({
  ...
  describe('OSV cache edge cases', () => {
    ...
    it('should prevent concurrent initialization race condition', async () => {
      ...
      const newCacheManager = new CacheManager(mockContext, mockLog, {
        vulnerabilityTTLMinutes: 60,
      });

      // Trigger multiple concurrent initialization calls
      const initPromises = Array.from({ length: 10 }, () =>
        newCacheManager.getCachedOSVVulnerabilities('test-package', '1.0.0')
      );

      // All should resolve without errors (race condition should be prevented)
      await expect(Promise.all(initPromises)).resolves.toBeDefined();

      // Verify createDirectory was only called once (not 10 times)
      const vscode = await import('vscode');
      const createDirectoryCalls = vi.mocked(vscode.workspace.fs.createDirectory).mock.calls;
      // Should be called once per cache manager instance (constructor + concurrent calls share same promise)
      expect(createDirectoryCalls.length).toBeGreaterThanOrEqual(1);
    });
  });
});
